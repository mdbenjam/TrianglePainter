...
class Brush:
    ...
    def clear_stroke(self, window, canvas):
        ...
        def prune_points(contour):
            ...
            def iterate_pruning():
                debug_print(['num contours = ', len(contour)])
                for c in contour:
                    if len(c) < 2:
                        continue
                    last_p = c[0]
                    ...
                    if contour_hole == False:
                        p1 = self.triangle_points[-1]
                        p2 = self.triangle_points[-2]
                        dy = p1[1] - p2[1]
                        dx = p1[0] - p2[0]
                        norm = np.linalg.norm([dy, dx])
                        dy /= norm
                        dx /= norm
                        holes.append([(p1[0]+p2[0])/2.0+dy*r, 
                                      (p1[1]+p2[1])/2.0-dx*r])
                        contour_hole = True
        ...
        def convert_pointset_to_contour(cont):
            contours = []
            debug_print([ 'primary number contours = ', len(cont)])
            for c in cont:
                if len(c) < 2:
                    debug_print([c])
                    continue
        ...
        def create_triangulation_arrays():
            ...
            holes_array = np.empty([len(saveHoles), 2])
            i = 0
            debug_print(['holes', saveHoles])
            for h in saveHoles:
                holes_array[i, 0] = h[0]
                holes_array[i, 1] = h[1]
                i = i + 1
            ...
            self.holesArray = holes_array

        create_triangulation_arrays()

        def triangulation_one():
            if 0: #len(self.holesArray) > 0:
                # glClear(GL_COLOR_BUFFER_BIT)
                # self.draw_stroke(black=True)
                ...
                # glColor3f(1, 0, 0)
                # glBegin(GL_POINTS)
                # for r in self.holesArray:
                #     glVertex2f(r[0], r[1])
                # glEnd()
                # canvas.SwapBuffers()
                # raw_input('press any key')

                A = dict(vertices = self.arrayPoints, segments = self.boundary, holes = self.holesArray)
            else:
        ...
        def triangulation_two():
            ...
            if foundIndex > -1:
                hole_array[i, 0] = hole_points[foundIndex][0]
                hole_array[i, 1] = hole_points[foundIndex][1]
                #hole_array[i, 0], hole_array[0, 0] = hole_array[0, 0], hole_array[i, 0]
                #hole_array[i, 1], hole_array[0, 1] = hole_array[0, 1], hole_array[i, 1]
                hole_array[i, 0] = 10#hole_array[0, 0]
                hole_array[i, 1] = 10#hole_array[0, 1]
                
                i += 1

            if 0:#self.strokeNumber == 2:
                glClear(GL_COLOR_BUFFER_BIT)
            ...
            if i == 0:
                ...
                # if self.strokeNumber == 2:
                #     print 'arraylen', len(hole_array)
                #     A = dict(vertices = verts, segments = line_array, holes = hole_array[0:13])
                # else:
                A = dict(vertices = verts, segments = line_array)#, holes = hole_array)
        ...
        real_segs = []
        for r in redo_segments:
            real_segs.append((convert_redo_point_to_composite(r[0]), convert_redo_point_to_composite(r[1])))
        ...
        for indices in tri_indicies:
            ...
            self.triangles.append(geometry.Triangle(t))
        self.triangles = geometry.remove_triangles_with_holes(self.triangles, hole_points, real_segs, self.composite_points, grid_x, grid_y, window, canvas)
    ...
    def open_record(self, in_name):
        ...
        while '$' in f.readline():
            ...
            nextStroke.color = [float(color[0]), float(color[1]), float(color[2]), float(color[3])]
            brushParams = f.readline().split(',')
            nextStroke.size = float(brushParams[0])
    ...
    def get_strokes_in_record(self):
